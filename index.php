...
//残り時間を計算
$sql_time="SELECT * FROM ra_content";
$query_time=mysqli_query($link,$sql_time);
$row_time=array();
while($a_time=mysqli_fetch_assoc($query_time)){
  $row_time[]=$a_time;
}
for($i=0;$i<count($row_time);$i++){
  $deadtime=strtotime($row_time[$i]['start_time'])+$row_time[$i]['ra_time']*24*60*60;
  $row[$i]['deadtime']=date("Y-m-d H:i:s",$deadtime);
}
//条件初期化
$con_sql="";
$con_sql1="";
...
$con=$con_sql.$con_sql1.$con_sql2;
if(isset($_SESSION)&&!empty($_SESSION)){//ログインの場合
  $logined_id=$_SESSION['mem_id'];
  $sql="SELECT * FROM (ra_content INNER JOIN genre ON ra_content.gen_id = genre.gen_id )INNER JOIN ra_demand ON ra_content.ra_id = ra_demand.ra_id WHERE ra_demand.mem_id !='{$logined_id}'  $con";
}
else{
  ...
  // echo $con;
  $sql="SELECT * FROM (ra_content INNER JOIN genre ON ra_content.gen_id = genre.gen_id )INNER JOIN ra_demand ON ra_content.ra_id = ra_demand.ra_id {$con}";
}
